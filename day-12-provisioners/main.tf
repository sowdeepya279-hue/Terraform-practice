...
resource "null_resource" "run_script" {
    ...
    connection {
     ...
     provisioner "file" {
    source      = "sow.sh"
    destination = "/home/ubuntu/sow.sh" #destination path on the remote instance copy the file10 from local to remote instance with the name file10
  }

  provisioner "remote-exec" {
    inline = [
       # "echo 'hello from veera Nareshit AIOPS MULTICLOUD DEVOPS' >> /home/ubuntu/file200",
      
        "bash /home/ubuntu/sow.sh" # Assuming sow.sh is already on the instance 
    ]
  }
...
#   }
 triggers = {
 script_hash = filemd5("sow.sh") # Rerun only if script changes
  }
   }
